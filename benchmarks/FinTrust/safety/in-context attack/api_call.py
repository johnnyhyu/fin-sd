...
async def process_one_item(topic, item, client, model, sem):
...
def get_jobs(input_path, model_key, sem, *, system_prompt_type=None):
    ...
    print(f"Loaded {total} prompts under {len(data)} topics.")
    ...
    tasks = []
    for topic, item_list in data.items():
        for item in item_list:
            tasks.append(process_one_item(topic, item, client, model, sem))
