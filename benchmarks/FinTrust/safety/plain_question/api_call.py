...
async def process_one_prompt(topic, prompt, client, model, sem):
    async with sem:
        try:
            ...
            return {
                "topic": topic,
                "plain_attack": prompt,
                "answer_to_plain_attack": answer
            }
        except Exception as e:
            return {
                "topic": topic,
                "plain_attack": prompt,
                "answer_to_plain_attack": f"ERROR: {str(e)}"
            }

def get_jobs(input_path, model_key, sem, *, system_prompt_type=None):
    print(f"Loading file: {input_path}")
    topic2prompts = load_json_dict(input_path)
    print(f"Loaded {len(topic2prompts)} topics.")
    ...
    tasks = []
    for topic, prompt_list in topic2prompts.items():
        for prompt in prompt_list:
            tasks.append(process_one_prompt(topic, prompt, client, model, sem))

    def writer(results):
        out_path = f"{os.path.splitext(input_path)[0]}.{model_key}.llm_plain_attack_output.json"
...
async def main():
    ...
    results = await tqdm_asyncio.gather(*tasks, desc="LLM answering", total=len(tasks))
