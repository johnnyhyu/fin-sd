...
async def generate_from_openai_chat_completion(
    ...
    for response in responses:
        if response is not None:
            ...
            outputs.append(response_dict['choices'][0]['message']['content'])
            if "reasoner" in engine_name or "r1" in engine_name:
                reasoning_contents.append(response_dict['choices'][0]['message']['reasoning_content'])
            else:
                reasoning_contents.append("")
            usages.append(response_dict['usage'])
        else:
            outputs.append("Invalid Message")
            reasoning_contents.append("")
            usages.append(None)

    return outputs, reasoning_contents, usages