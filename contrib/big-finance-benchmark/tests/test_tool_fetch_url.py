...
def test_describe_exception_names_the_class_when_httpx_has_no_message():
    """Transport failures carry NO message: `ConnectError`, `ConnectTimeout`,
    `ReadTimeout`, `ReadError` and `RemoteProtocolError` all stringify to '' — the class
    is the whole diagnosis. Interpolated naively that produced a tool result reading
    exactly `fetch_url failed:`, with no URL and no cause, and models responded by
    re-issuing the identical call: 27 of the 50-question subset's fetch errors were
    that loop."""
    import httpx

    from big_finance_harness.tools.fetch_url import _describe_exception
    ...
    for cls in (
        ...
        exc = cls("")
        assert str(exc) == "", f"{cls.__name__} now carries a message; test is stale"
        assert _describe_exception(exc) == cls.__name__


def test_describe_exception_keeps_the_detail_when_there_is_one():
    """Naming the class must not cost the message when httpx supplies one."""
    import httpx

    from big_finance_harness.tools.fetch_url import _describe_exception

    described = _describe_exception(httpx.ConnectError("nodename nor servname provided"))
    assert described == "ConnectError: nodename nor servname provided"


@pytest.mark.asyncio
async def test_fetch_url_surfaces_the_class_in_the_tool_error():
    """End-to-end at the raise site: a messageless transport error must not reach the
    model as a bare `fetch_url failed:`."""
    import httpx
    ...
    tool._fetch = _boom  # bypass the retry decorator; we are testing the handler

    with pytest.raises(ToolError) as exc:
        await tool.run({"url": "https://example.com/unreachable"})

    message = str(exc.value)
    assert "ConnectTimeout" in message
    assert message.rstrip() != "fetch_url failed:"
