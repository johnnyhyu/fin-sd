...
LiteLLM populates `_hidden_params["response_cost"]` for most providers, but three
...
2.  Eval phase: locally self-hosted vLLM routes (`local:<checkpoint>`, e.g.
    gpt-oss-120b) have no upstream price at all, so they record `cost_usd=None`;
    a flat per-token rate (`_LOCAL_TOKEN_RATE`) is applied.
3.  Judge phase: some Vertex preview snapshots (e.g. Gemini 3.1 Pro Preview on
...
from big_finance_harness.trace import jsonl_lines, read_traces
...
_GATEWAY_PRICING: dict[str, dict[str, float]] = {
    ...
    # Slugs are keyed both as the gateway names them and as OpenRouter does, since the
    # lineup moved to OpenRouter routes: a trace records `z-ai/glm-5.1`, which would
    # never have matched a `zai/` key, so those rows silently fell through to
    # `cost_source="unknown"` and dropped out of the cost table.
    "zai/glm-5.1": {
        "input": 0.0000014,
        "output": 0.0000044,
    ...
    # Self-serve flat rate: $0.17 / 1M tokens, input and output alike.
    "openai/gpt-oss-120b": {
        "input": 0.00000017,
        "output": 0.00000017,
    },
...
# Flat blended rate for locally self-hosted vLLM routes (`local:<checkpoint>`), which
# LiteLLM cannot price. $0.17 / 1M tokens, applied to both prompt and completion —
# the same figure used for gpt-oss-120b above, since local runs serve that checkpoint.
_LOCAL_TOKEN_RATE = 0.00000017
...
_JUDGE_RATES: dict[str, dict[str, float]] = {
    "openrouter:google/gemini-3.1-pro-preview": {"input": 2e-6, "output": 12e-6},
    "openrouter:anthropic/claude-opus-4.7": {"input": 5e-6, "output": 25e-6},
    "openrouter:anthropic/claude-sonnet-4.6": {"input": 3e-6, "output": 15e-6},
}


def _route_prefixes(run_dir: Path) -> dict[str, str]:
    """Map trace label -> the provider prefix the run configured for it.

    `RunRecord.model` stores only the snapshot (`moonshotai/kimi-k2.6`), so a trace on
    its own cannot say whether it came from `gateway:` or `openrouter:`. The manifest
    keeps the full id, and without it every OpenRouter trace whose label happens to
    appear in `_GATEWAY_PRICING` got billed at gateway rates and stamped
    `cost_source="recomputed_gateway"` — a wrong number wearing an authoritative label.
    """
    ...
    for entry in manifest.get("models") or []:
        label, model_id = entry.get("label"), entry.get("model_id") or ""
        if label and ":" in model_id:
            out[label] = model_id.split(":", 1)[0]
    return out
...
def _compute_local_cost(prompt_tokens: int, completion_tokens: int) -> float:
    """Flat-rate cost for a locally self-hosted vLLM trace. No cache-read column —
    every token (prompt + completion) is billed at `_LOCAL_TOKEN_RATE`."""
    return (prompt_tokens + completion_tokens) * _LOCAL_TOKEN_RATE
...
def _run_eval_phase(run_dir: Path, out_path: Path) -> None:
    ...
    route_prefixes = _route_prefixes(run_dir)
    for traces_path in sorted(run_dir.glob("*.traces.jsonl")):
        label = traces_path.stem.removesuffix(".traces")
        # Absent a manifest we can't tell which table applies, so fall back to the old
        # permissive behaviour rather than dropping costs for a hand-assembled run dir.
        prefix = route_prefixes.get(label)
        gateway_priced = prefix in (None, "gateway")
        for r in read_traces(traces_path):
            cost = r.cost_usd
            source = "litellm"
            if cost is None:
                gw_model = _gateway_model_from_resolved(r.resolved_model, r.model)
                if gw_model is not None and gateway_priced:
                ...
                elif (r.model or "").startswith("local:"):
                    # Locally self-hosted vLLM route — flat per-token rate.
                    cost = _compute_local_cost(
                        r.total_prompt_tokens, r.total_completion_tokens
                    )
                    source = "recomputed_local"
                else:
                    source = "unknown"
                if source in ("recomputed_gateway", "recomputed_local"):
                    n_recomputed += 1
                else:
                    n_unknown += 1
            else:
...
def _run_judge_phase(run_dir: Path, out_path: Path) -> None:
    ...
    for grades_path in sorted(set(run_dir.glob("*.grades*.jsonl"))):
        ...
        if ".grades." in stem:
            ...
            continue
        for line in jsonl_lines(grades_path):
            if not line.strip():
                continue
            try:
                g = json.loads(line)
