...
def _raters_are_degenerate(pairs: list[tuple[bool, bool]]) -> bool:
    """True when both raters emitted a single constant value across every observation.

    κ is undefined there (expected agreement is 1, so the statistic is 0/0), and
    `_cohen_kappa` returns 1.0 by convention. That is a trap for a headline table: the
    run in `runs/headline/` produced `kappa,1.0` because *both* judges scored every one
    of 150 answers incorrect — no agreement information at all, printed as perfect
    agreement. The κ value is left as-is so existing numbers don't move; this flag is
    surfaced alongside it so the 1.0 is never read without the caveat.
    """
    if not pairs:
        return False
    a_values = {a for a, _ in pairs}
    b_values = {b for _, b in pairs}
    return len(a_values) == 1 and len(b_values) == 1
...
def main(per_grade_csv: Path, out_dir: Path) -> None:
    ...
    with kappa_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for model, key_to_judges in sorted(by_model.items()):
            ...
            degenerate = _raters_are_degenerate(pairs)
            if degenerate:
                click.echo(
                    f"  warning: {model}: both judges returned the same constant verdict "
                    f"on all {len(pairs)} paired observations — κ is undefined here, and "
                    f"the reported {kappa:.1f} carries no agreement information.",
                    err=True,
                )
            w.writerow(
                {
