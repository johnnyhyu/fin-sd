...
Results that point at the benchmark's own published dataset/paper are dropped before the
model sees them — see `big_finance_harness.contamination` for why.
"""
...
from big_finance_harness.contamination import filter_search_results
...
class WebSearchTool(Tool):
    ...
    async def run(self, args: dict[str, Any]) -> str:
        query = require_str(args, "query", 'expected {"query": "<search terms>"}')
        try:
            results = await self.backend.search(query, self.max_results)
        except httpx.HTTPError as e:
            raise ToolError(f"web_search failed: {e}") from e
        # Drop the benchmark's own dataset/paper before the model sees it. Reported in
        # the payload so the exclusion is visible in the trace rather than invisible.
        results, n_excluded = filter_search_results(results)
        ...
        if n_excluded:
            payload["excluded_benchmark_results"] = n_excluded
            payload["note"] = (
                "Some results were withheld: they are this benchmark's own published "
                "dataset or paper, which contain the reference answers. Use primary "
                "sources (SEC filings, press releases, investor-relations pages)."
            )
