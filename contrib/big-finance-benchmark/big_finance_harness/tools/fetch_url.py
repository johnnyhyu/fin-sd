...
from big_finance_harness.contamination import LEAK_FETCH_MESSAGE, is_benchmark_source
...
# Redirect hops we'll follow. SEC `Archives/` URLs and IR sites redirect one or two
# times; five leaves headroom without letting a loop run away.
MAX_REDIRECTS = 5
...
def _check_url_safe_sync(url: str) -> None:
    ...
    try:
        ...
        # `urlparse` defers port validation to attribute access, so a URL like
        # `http://x:notaport/` raises here rather than at parse time.
        raise ToolError(f"fetch_url got an invalid port in {url!r}: {e}") from e
    try:
...
async def _check_url_safe(url: str) -> None:
    """Async wrapper for `_check_url_safe_sync`.

    `socket.getaddrinfo` is a blocking C call. Run inline on the event loop it stalls
    every other concurrent question in the process for the duration of the DNS lookup —
    at `concurrency=8` on a cold resolver that is a real throughput tax, and on a
    DNS timeout it freezes the whole orchestrator. Push it to a worker thread.
    """
    await asyncio.to_thread(_check_url_safe_sync, url)


def _describe_exception(exc: BaseException) -> str:
    """Render an exception as text that still says something when `str(exc)` is ''.

    Most httpx transport failures carry NO message: `ConnectError`, `ConnectTimeout`,
    `ReadTimeout`, `ReadError` and `RemoteProtocolError` all stringify to the empty
    string, because the class itself is the whole diagnosis. Interpolated naively that
    produced a tool result reading exactly `fetch_url failed:` — a dead end with no
    URL, no cause, and nothing to act on.

    The models treated it as such: on the 50-question subset, 27 of the fetch errors
    were a model re-issuing the *identical* call against a bare `fetch_url failed:`,
    burning steps on a fetch that was never going to succeed. Naming the class turns it
    into "fetch_url failed: ConnectTimeout", which is enough to tell a timeout from a
    DNS failure from a protocol error, and enough to make the model try another source.
    """
    text = str(exc).strip()
...
def _html_to_text(html: str) -> str:
    # SEC serves plenty of XML (XBRL instance docs, filing indexes, RSS) that we still
    # want as text. bs4 emits a 10-line XMLParsedAsHTMLWarning for each one; across a
    # 928-question run that buries the orchestrator's own output, including the run
    # health warnings. The HTML parser handles these documents fine for text extraction.
...
class FetchUrlTool(Tool):
    ...
    async def _fetch(self, url: str) -> httpx.Response:
        ...
        # Redirects are followed manually so every hop can be validated. With httpx's
        # automatic `follow_redirects=True`, only the *first* URL is checked — a
        # 302 from an attacker-influenced page (or from a prompt-injected tool result)
        # is enough to reach cloud metadata or a localhost service, and the request
        # lands before any post-hoc inspection can stop it.
        async with httpx.AsyncClient(timeout=self.timeout_s, follow_redirects=False) as client:
            ...
            for _ in range(MAX_REDIRECTS + 1):
                ...
                await resp.aclose()
                if is_benchmark_source(next_url):
                    raise ToolError(LEAK_FETCH_MESSAGE.format(url=next_url))
                await _check_url_safe(next_url)
                request = resp.next_request
            raise ToolError(
                f"fetch_url exceeded {MAX_REDIRECTS} redirects starting from {url!r}"
            )

    async def run(self, args: dict[str, Any]) -> str:
        url = require_str(args, "url", 'expected {"url": "https://www.sec.gov/..."}')
        if is_benchmark_source(url):
            raise ToolError(LEAK_FETCH_MESSAGE.format(url=url))
        await _check_url_safe(url)
        query = args.get("query")
        try:
            max_tokens = int(args.get("max_tokens") or self.default_max_tokens)
        except (TypeError, ValueError):
            # A non-numeric `max_tokens` shouldn't cost the model a step; fall back to
            # the default rather than erroring out the whole call.
            max_tokens = self.default_max_tokens

        try:
            resp = await self._fetch(url)
        except httpx.HTTPError as e:
            raise ToolError(f"fetch_url failed: {_describe_exception(e)}") from e
