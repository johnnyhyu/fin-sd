...
class Message(BaseModel):
    ...
    # The provider's chain of thought for an assistant turn, carried so it can
    # optionally be replayed to the server on the next request. Only actually sent
    # when BFH_REPLAY_REASONING is set — see `models/base._replay_reasoning`, which
    # is also where the reason it is off by default is written down.
...
class ModelResponse(BaseModel):
    ...
    text: str
    # Provider-surfaced chain of thought (LiteLLM's `reasoning_content`). Reasoning
    # models put nearly everything here and leave `text` empty until the final turn, so
    # dropping it loses the analyst work the rubric is written against.
...
class StepRecord(BaseModel):
    ...
    step: int
    assistant_text: str
    # Mirrors `ModelResponse.reasoning_content`; see the note there.
